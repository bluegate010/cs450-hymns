...
/**
 * <p>
 * The {@code SearchResults} class is a fixed-size {@link PriorityQueue}
 ...
 * 
 * <ol>
 * <li>The {@code SearchResults} instance contains fewer points than its maximum
 * capacity <em>or</em> the offered point is closer to the query point than the
 * most distant point already in the queue.</li>
 ...
 * 
 * <p>
 * {@code SearchResults} objects use an internal reverse geospatial distance
...
public class SearchResults<E extends SpatialPoint> extends PriorityQueue<E> {
	 ...
	 */
	public SearchResults(SpatialPoint queryPoint, int maxSize,
			double maxDistance, SearchCriteria<E> criteria) {
		super(maxSize, java.util.Collections
				.reverseOrder(new SpatialDistanceComparator<E>(queryPoint)));
	...
	public boolean add(E point) {
		...
		if (this.size() < this.maxSize) {
			...
			if (this.maxDistance == Double.POSITIVE_INFINITY
					|| this.queryPoint.getDistanceTo(point) <= this.maxDistance) {
			...
			// maximum range.
			if (this.queryPoint.getDistanceTo(point) < this
	...
	public boolean addAll(Collection<? extends E> points) {
		if (this.equals(points)) {
			throw new IllegalArgumentException(
					"Cannot addAll of a queue to itself.");
		}
	...
	public List<E> toSortedList() {
		...
		java.util.Collections.sort(sortedList,
				new SpatialDistanceComparator<E>(this.queryPoint));
